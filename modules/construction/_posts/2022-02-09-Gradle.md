...
folder inside of your project named `NBA-Excel-1.3.jar`.

You can now `cd` to that folder and run that .jar file with:

`java -jar NBA-Excel-1.3.jar`
...
The files `build.gradle.kts`, `settings.gradle.kts`, `gradlew`, and `gradlew.bat` are files
needed for Gradle to function correctly. The most interesting of these is
`build.gradle.kts` which acts as a type of "script" for how your project is built.
`settings.gradle.kts` is also worth looking at briefly. We will look at both of these
...
This file acts as the beating heart of the Gradle build. Let's take a look at its contents
piece by piece. You can see the [build file for this project on Github.](https://github.com/sde-coursepack/NBAExcelTeams/blob/main/build.gradle.kts)

The build.gradle file is written in a **domain-specific language** using Kotlin.
However, you don't need to learn Kotlin to work with Gradle, as it's
designed to be very readable and modifiable.

Note that there is also a Groovy DSL for Gradle. To tell which you are working with, check your build file name. If the file name is:  
* `build.gradle` - This is using the Gradle Groovy DSL
* `build.gradle.kts` - This is using the Gradle Kotlin DSL

In general, if choosing between the two, I would recommend the Kotlin DSL, as this is the standard for popular software platforms like Android, Jetpack Compose, etc. However, the differences are purely syntactic, as neither DSL does anything the other cannot do.
...
* junit-platform-launcher - used at runtime to launch the JUnit tests  
...
Central repository under `org.apache.poi`. Please get version `5.5.1`."
...
* `runtimeOnly` - I only need this resource at runtime, but my source code does not use this resource directly  
...
`group` is useful if we want to deploy our .jar file on Maven or something like that. I tend to just set this to the package name that contains my `Main` class, or whichever class contains the `main` function.
...
`manifest` is where we define the properties of our jar Manifest. Whenever you are releasing a **runnable** jar, then you **absolutely must include** the
`attributes[Main-class]` item. This should be set to the full package and class name of your main class. In my code, the `main` method is in `Main.java` in the package `edu.virginia.cs.nbateams`.
...
the dependencies in my jar file, I would need to separately run the `java -jar` command with additional arguments to specify where to find those dependencies. While doable, this is complicated and beyond what we need in this class. Inside of the `tasks.jar` closure, the purpose of:
...
...is simply to tell Gradle "Hey, include all the dependencies in the jar file." You will want to check the licenses of any libraries you use to ensure that you have a right to redistribute them. For the libraries I have selected for homework assignments, I have ensured that the License allows redistribution.
...
external libraries, is going to be described in the Gradle build file.
